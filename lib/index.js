 ...
 * @since v0.1.0
 */
const throwIfError = require('./bindings/util/throwIfError')
...
module.exports.setKeepAliveInterval = function setKeepAliveInterval(
  ...
  const [err, value] = platform.setKeepAliveInterval(socket, msecs)
  throwIfError(err)
  return value
}
...
module.exports.getKeepAliveInterval = function getKeepAliveInterval(socket) {
  const [err, value] = platform.getKeepAliveInterval(socket)
  throwIfError(err)
  return value
}
...
module.exports.setKeepAliveProbes = function setKeepAliveProbes(socket, cnt) {
  const [err, value] = platform.setKeepAliveProbes(socket, cnt)
  throwIfError(err)
  return value
}
...
module.exports.getKeepAliveProbes = function getKeepAliveProbes(socket) {
  const [err, value] = platform.getKeepAliveProbes(socket)
  throwIfError(err)
  return value
}
...
module.exports.setUserTimeout = function setUserTimeout(socket, msecs) {
  const err = platform.setUserTimeout(socket, msecs)
  throwIfError(err)
  return true
}
...
module.exports.getUserTimeout = function getUserTimeout(socket) {
  const [err, value] = platform.getUserTimeout(socket)
  throwIfError(err)
  return value
}
