...
describe('tcp-dump', function() {
  ;(process.getuid && process.getuid() === 0 ? it : it.skip)(
    'should send keepalive packets on the wire',
    function(done) {
      this.timeout(10000)
      ...
      Assert.ok(
        tcpDumpIface,
        'could not detect internal (loopback) interface name'
      )
      ...
      Net.createServer().listen(0, function() {
        ...
        var s = Net.createConnection(addr, function() {
          ...
          var tcpdump = ChildProcess.spawn(
            '/usr/sbin/tcpdump',
            ['-c', '10', '-i', tcpDumpIface, tcpdumpFilter],
            { stdio: 'inherit' }
          )
